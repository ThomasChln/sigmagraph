...
sigma_mutuals = function(igraph, color_map = NULL, point_size = 4, niter = 5e3,
  ...
  if (!is.null(node_size)) {
      if (is.numeric(node_size)) {
        sigma_graph %<>% add_node_size(oneSize = node_size)
      } else {
        sigma_graph %<>% add_node_size(sizeVector = igraph::vertex_attr(igraph, node_size))
      }
  }
...
add_igraph_info = function(igraph, df_nodes, fields = c('word', 'clusters')) {
